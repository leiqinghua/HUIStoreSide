...
#import "HLVoucherBuyTableCell.h"
...
@interface HLVoucherBuyTableCell () <HLOrderOpetionDelegate>

@property (nonatomic, strong) HLVoucherBuyLayout *voucherLayout;

@end

@implementation HLVoucherBuyTableCell
...
#pragma mark - getter
- (void)setOrderLayout:(HLVoucherBuyLayout *)orderLayout {
    [super setOrderLayout:orderLayout];
    _voucherLayout = orderLayout;
...
- (UITableViewCell *)hl_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //商品部分
    if (indexPath.section == self.voucherLayout.stmDesSection) {
        //展示 价格说明
        if (orderModel.settlementDes.count && indexPath.row == self.voucherLayout.stmRows - 1) {
...
- (CGFloat)hl_tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        return self.voucherLayout.orderInfoHight;
    }
    
    if (indexPath.section == 1) {
        return self.voucherLayout.userInfoHight;
    }
    
    if (indexPath.section == self.voucherLayout.stmDesSection) {
        //展示 价格说明
        BOOL descRow = self.voucherLayout.orderModel.settlementDes.count && indexPath.row == self.voucherLayout.stmRows - 1;
        if (descRow) {
            return [self.voucherLayout calculatePriceDescHight];
        }
        return self.voucherLayout.goodsHight;
    }

    return self.voucherLayout.bottomHight;
}


- (UIView *)hl_tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if (section == self.voucherLayout.stmDesSection) {
...
- (CGFloat)hl_tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if (section == self.voucherLayout.stmDesSection) {
        return self.voucherLayout.footerHight;
    }
    return 0.0;
}
