...
@interface HLHotBuySendRangeController () <UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,HLSendOrderRangeHeadDelegate>
...
@implementation HLHotBuySendRangeController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"选择配送区域";
...
- (void)loadData{
    ...
    [XMCenter sendRequest:^(XMRequest * _Nonnull request) {
        ...
        request.api = @"/shopplus/hotgoods/sendRange";
...
- (void)saveButtonClick{
    ...
    if (self.isSelectCustom) {
        if (self.numInput.text.integerValue <= 0) {
            HLShowHint(@"配送范围必须大于0", self.view);
            return;
        }
        range = self.numInput.text.integerValue;
    }else{
        // 判断是否有勾选
        ...
        range = info.itemId.integerValue;
    }
    
    if (self.callBack) {
        self.callBack(self.isSelectCustom, range);
    }
