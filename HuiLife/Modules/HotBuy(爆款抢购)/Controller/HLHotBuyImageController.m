...
@interface HLHotBuyImageController () <UICollectionViewDelegate,UICollectionViewDataSource,HLHotSekillImageViewCellDelegate>
...
@implementation HLHotBuyImageController

#pragma mark - LifeCycle

- (void)viewWillAppear:(BOOL)animated{
    [super viewWillAppear:animated];
    [self hl_setTitle:@"爆款商品图册" andTitleColor:UIColorFromRGB(0x333333)];
}
...
- (void)saveButtonClick{
    ...
    for (HLHotSekillImageModel *imageModel in self.dataSource[0]) {
        ...
        if (imageModel.isFirst) {
            firstImage = imageModel.image;
        }
        
        [picArr addObject:@{@"pic":imageModel.imgUrl,@"is_check":imageModel.isFirst ? @1 : @0}];
    }
    ...
    for (HLHotSekillImageModel *imageModel in self.dataSource[1]) {
        ...
        [detailPicArr addObject:imageModel.imgUrl];
    }
    ...
    NSInteger count = picArr.count + detailPicArr.count;
    if (self.callBack) {
        self.callBack([picArr mj_JSONString], [detailPicArr mj_JSONString], firstImage, count);
    }
...
- (HLHotSekillImageModel *)buildSekillImageModelWithImage:(UIImage *)image{
    ...
    imageModel.saveName = @"upload";
    imageModel.isStoreService = YES;
    imageModel.postParams = @{};
    imageModel.uploadedImgUrlKey = @"path";
    imageModel.requestApi = @"/shopplus/hotgoods/upload";
    return imageModel;
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    header.title = indexPath.section == 0 ? @"爆款主图" : @"商品详情图";
    return header;
}
