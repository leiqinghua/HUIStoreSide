...
@implementation NSObject (HLExtension)
...
- (BOOL)isEmptyArr{
    if ([self isKindOfClass:[NSArray class]]){
        for (NSString * item in (NSArray *)self) {
            if ([item hl_isAvailable]) {
                return NO;
            }
        }
    }
...
+ (NSArray *)allValuesWithDictionary:(NSDictionary *)dict{
    NSArray *keys = [self allKeysWithDictionary:dict];
