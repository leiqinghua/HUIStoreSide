...
#import "HLDiscountBuyTableCell.h"
...
@interface HLDiscountBuyTableCell () <HLOrderOpetionDelegate>

@property (nonatomic, strong) HLDiscountBuyLayout *discountLayout;

@end


@implementation HLDiscountBuyTableCell
...
#pragma mark - getter
- (void)setOrderLayout:(HLDiscountBuyLayout *)orderLayout {
    [super setOrderLayout:orderLayout];
    _discountLayout = orderLayout;
...
- (UITableViewCell *)hl_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) { //订单信息
        ...
        cell.orderModel = self.discountLayout.orderModel;
        return cell;
    }
    ...
    //商品部分
    if (indexPath.section == self.discountLayout.stmDesSection) {
        //展示 价格说明
        BOOL descRow = self.discountLayout.orderModel.settlementDes.count && indexPath.row == self.discountLayout.stmRows - 1;
        if (descRow) {
            ...
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.contents = self.discountLayout.orderModel.settlementDes;
            return cell;
        }
        //显示商品
        HLGoodTableCell *cell = (HLGoodTableCell *) [tableView hl_dequeueReusableCellWithIdentifier:@"HLGoodTableCell" indexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.goodModel = self.discountLayout.orderModel.Info[indexPath.row];
        return cell;
    }
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.contents = self.discountLayout.orderModel.bottomContents;
    return cell;
}
...
- (CGFloat)hl_tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        return self.discountLayout.orderInfoHight;
    }
    
    if (indexPath.section == 1) {
        return self.discountLayout.userInfoHight;
    }
    ...
    if (indexPath.section == self.discountLayout.stmDesSection) {
        ...
        BOOL descRow = self.discountLayout.orderModel.settlementDes.count && indexPath.row == self.discountLayout.stmRows - 1;
        if (descRow) {
            return [self.discountLayout calculatePriceDescHight];
        }
        return self.discountLayout.goodsHight;
    }

    return self.discountLayout.bottomHight;
}


- (UIView *)hl_tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if (section == self.discountLayout.stmDesSection) {
...
- (CGFloat)hl_tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if (section == self.discountLayout.stmDesSection) {
        return self.discountLayout.footerHight;
    }
    return 0.0;
}
