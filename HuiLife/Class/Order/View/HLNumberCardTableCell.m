...
#import "HLNumberCardTableCell.h"
...
@interface HLNumberCardTableCell () <HLOrderOpetionDelegate>

@property (nonatomic, strong) HLNumberCardLayout *numberLayout;

@end

@implementation HLNumberCardTableCell
...
#pragma mark - getter
- (void)setOrderLayout:(HLNumberCardLayout *)orderLayout {
    [super setOrderLayout:orderLayout];
    _numberLayout = orderLayout;
...
- (UITableViewCell *)hl_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //商品部分
    if (indexPath.section == self.numberLayout.stmDesSection) {
        //展示 价格说明
        if (orderModel.settlementDes.count && indexPath.row == self.numberLayout.stmRows - 1) {
...
- (CGFloat)hl_tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        return self.numberLayout.orderInfoHight;
    }
    
    if (indexPath.section == 1) {
        return self.numberLayout.userInfoHight;
    }
    
    if (indexPath.section == self.numberLayout.stmDesSection) {
        //展示 价格说明
        BOOL desRow = self.numberLayout.orderModel.settlementDes.count && indexPath.row == self.numberLayout.stmRows - 1;
        if (desRow) {
            return [self.numberLayout calculatePriceDescHight];
        }
        return self.numberLayout.goodsHight;
    }
    return self.numberLayout.bottomHight;
}


- (UIView *)hl_tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if (section == self.numberLayout.stmDesSection) {
...
- (CGFloat)hl_tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if (section == self.numberLayout.stmDesSection) {
        return self.numberLayout.footerHight;
    }
    return 0.0;
}
