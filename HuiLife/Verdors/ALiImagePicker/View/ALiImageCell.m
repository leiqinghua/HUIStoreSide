...
//    self.asset.selected = self.checkBtn.isSelected;
    if ([self.delegate respondsToSelector:@selector(imageDidSelect:select:)]) {
        [self.delegate imageDidSelect:self.asset select:self.checkBtn];
    }
}
...
- (UIButton *)checkBtn
{
    if (_checkBtn == nil) {
        ...
        UIImage  *img = [UIImage imageNamed:@"circle_normal"];
        UIImage  *imgH = [UIImage imageNamed:@"success_oriange"];
