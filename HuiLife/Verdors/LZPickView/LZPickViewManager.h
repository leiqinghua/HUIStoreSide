 ...
 *  dateStringBlock 选择日期回调
 注意：显示默认显示年，月，日， 传入的max and min  一定是 yyyy-MM-dd 格式，否则会报错
 */
...
/// 显示 年，月 picker
/// @param maxString 最大时间
/// @param minString 最小时间
/// @param dateStringBlock 选择日期回调
//注意：显示默认显示年，月 传入的max and min  一定是 yyyy-MM 格式，否则会报错
- (void)showYearMonthWithMax:(NSString *)maxString withMinDateString:(NSString *)minString didSeletedDateStringBlock:(void (^)(NSString *dateString))dateStringBlock;
