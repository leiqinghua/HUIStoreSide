...
//弹框的高度
//static CGFloat backViewH = 258.0;//大致是键盘的高度
...
- (void)initSubView {
    self.frame = CGRectMake(0, 0, kPICKVIEW_SCREEN_WIDTH, kPICKVIEW_SCREEN_HEIGHT);
...
//
//        _pickerType = LZCustomLimitDateYAM;
//
//        [self initMaskView];
//        [self initTopView];
//
...
- (void)initContainerView{
    ...
    self.containerView = [[UIView alloc] initWithFrame:CGRectMake(0, kPICKVIEW_SCREEN_HEIGHT - 226-self.topView.frame.size.height, kPICKVIEW_SCREEN_WIDTH, 226+self.topView.frame.size.height)];
...
- (void)setViews{
    ...
    CGRect leftFrame = CGRectMake(leftMargin, self.topView.bounds.size.height, 70, 226);
    CGRect middleFrame = CGRectMake(CGRectGetMaxX(leftFrame)+margin,self.topView.bounds.size.height, 60, 226);
    CGRect rightFrame = CGRectMake(CGRectGetMaxX(middleFrame)+margin, self.topView.bounds.size.height, 60, 226);
    //年
    //时间选择器
    yearPicker = [[UIPickerView alloc]init];
    yearPicker.frame = leftFrame;
    yearPicker.delegate = self;
    yearPicker.dataSource = self;
    //月
    monthPicker = [[UIPickerView alloc]init];
    monthPicker.frame = middleFrame;
    monthPicker.delegate = self;
    yearPicker.dataSource = self;
    //日
    dayPicker = [[UIPickerView alloc]init];
    dayPicker.frame = rightFrame;
    dayPicker.delegate = self;
    dayPicker.dataSource = self;
    
    if (_pickerType == LZCustomLimitDateYear) { //只显示年
        yearPicker.frame = middleFrame;
        monthPicker = nil;
        dayPicker = nil;
    }
    
    if (_pickerType == LZCustomLimitDateYAM) { //年，月
        monthPicker.frame = rightFrame;
        dayPicker = nil;
    }
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    rowLabel.textColor = [UIColor hl_StringToColor:@"#656565"];
...
- (void)sureAction{
    ...
    if (_pickerType == LZCustomLimitDateYear) {
        dateStr = yearStr;
    } else if (_pickerType == LZCustomLimitDateYAM) {
...
//    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    [dateFormatter setDateFormat:[self dateFormate]];
...
//    装配数据
    ...
    if (_pickerType == LZCustomLimitDateDefault) {
        [yearPicker reloadAllComponents];
        [monthPicker reloadAllComponents];
        [dayPicker reloadAllComponents];
    } else if (_pickerType == LZCustomLimitDateYear) {
        [yearPicker reloadAllComponents];
    } else {
        [yearPicker reloadAllComponents];
        [monthPicker reloadAllComponents];
    ...
    HLLog(@"currentDate %@ birthDay %@",currentDateStr,self.minimumDate);
    ...
    }else if (![self haveNull:maxString] )
    {
        ...
        HLLog(@"maxstring  存在值 minstring为空");
        ...
        HLLog(@"maxstring  为空 minstring存在值");
    }else{
        HLLog(@"else ==%@ %@ ",maxString,minString);
    }
