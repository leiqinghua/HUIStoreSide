...
-(id)initWithFrame:(CGRect)frame style:(NSString *)style
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.heightScale = 1;
        self.lineImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"scan_oriange_line" inBundle:[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"resource" ofType: @"bundle"]] compatibleWithTraitCollection:nil]];
...
- (void)layoutSubviews {
    ...
    _lineImageView.frame = CGRectMake(Left, YMinRetangle + 2, sizeRetangle.width - 4, 2);
...
- (void)startAnimating {
    ...
    CGRect initFrame = CGRectMake(Left, YMinRetangle + 2, sizeRetangle.width - 4, 2);
...
//    CGContextSetStrokeColorWithColor(context, [UIColor colorWithRed:0.110 green:0.659 blue:0.894 alpha:1.00].CGColor);
    //#
    CGContextSetStrokeColorWithColor(context, UIColorFromRGB(0xFFFF9900).CGColor);
