...
static const void *zw_placeHolder_center_Key;
...
- (void)zwPlaceHolder_swizzling_layoutSubviews {
    if (self.zw_placeHolder) {
        ...
        if (self.placeholder_center) {
            self.zw_placeHolderLabel.center = CGPointMake(self.zw_placeHolderLabel.center.x, self.bounds.size.height / 2);
        }
    }
...
- (BOOL)placeholder_center{

    return objc_getAssociatedObject(self, &zw_placeHolder_center_Key);
}

-(void)setPlaceholder_center:(BOOL)placeholder_center{
    objc_setAssociatedObject(self, &zw_placeHolder_center_Key, @(placeholder_center), OBJC_ASSOCIATION_ASSIGN);
}
